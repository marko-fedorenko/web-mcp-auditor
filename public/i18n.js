...
const D = {
  en: {
    ...
    fixtureDemo: 'Open live demo (task tracker) ↗',
    fixtureImp: 'Imperative-only',
    fixtureDecl: 'Declarative-only',
    fixtureMixed: 'Mixed/bad',
    examplesHint: 'The 3 fixtures fill the URL field for auditing. The live demo opens in a new tab — it\'s a real Web MCP page you can interact with, then come back and audit it.',
  ...
  uk: {
    ...
    fixtureDemo: 'Відкрити живе демо (task tracker) ↗',
    fixtureImp: 'Тільки imperative',
    fixtureDecl: 'Тільки declarative',
    fixtureMixed: 'Mixed/bad',
    examplesHint: 'Три фікстури заповнюють поле URL для аудиту. Живе демо відкривається в новій вкладці — це справжня сторінка з Web MCP, з якою можна взаємодіяти, а потім повернутись і запустити аудит.',
